...
int	resched()
{
	...
	/* write pages to disk in case of dirty pages*/
	int frame, status, store, pageth;
	for (frame=0; frame<NFRAMES; frame++) {
		if (frm_tab[frame].fr_type==FR_PAGE && frm_tab[frame].fr_pid == old_pid ) {
			status =  bsm_lookup(old_pid, (frm_tab[frame].fr_vpno)*NBPG, &store, &pageth);
			if (status != SYSERR){
				write_bs((frame+FRAME0)*NBPG, store, pageth);
			}
		}
		
		if (frm_tab[frame].fr_type==FR_PAGE && frm_tab[frame].fr_pid==currpid){
			status = bsm_lookup(currpid, (frm_tab[frame].fr_vpno)*NBPG, &store, &pageth);
			if (status != SYSERR){
