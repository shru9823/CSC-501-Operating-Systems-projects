...
nulluser()				/* babysit CPU when no one is home */
{
	...
	sysinit();
	paginginit();	/*	Initialize paging data structure */
...
/*------------------------------------------------------------------------
 *  paginginit  --  initialize paging data structures
 *------------------------------------------------------------------------
 */
LOCAL
paginginit()
{
	SYSCALL pfintr();
	init_bsm (); 								/* init bs map    		*/
	init_frm ();								/* init frame map 		*/
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	// Initialize with dummy parameters for NULL Process Entry
	// head_FIFO.frameID = -1;
